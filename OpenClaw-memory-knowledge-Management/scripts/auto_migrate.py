...
# 确保同目录下的模块可被 import（无论 cwd 是否为 scripts/）
...
def acquire_lock(target_root: Path) -> Optional[Path]:
    ...
    for _attempt in range(2):
        try:
            ...
            if _attempt == 0 and _is_stale_lock(lock_file):
                try:
                    lock_file.unlink()
                except OSError:
                    return None
...
def run_cmd(cmd: list[str], quiet: bool, env: Optional[Dict[str, str]] = None) -> Tuple[int, str, str]:
    if not quiet:
...
def main() -> int:
    ...
    try:
        ...
        migrate_rc, migrate_stdout, migrate_stderr = run_cmd(cmd, quiet=args.quiet, env=subprocess_env)
        if migrate_rc != 0:
        ...
        if run_evolve:
            ...
            evolve_rc, evolve_stdout, evolve_stderr = run_cmd(evolve_cmd, quiet=args.quiet, env=subprocess_env)
