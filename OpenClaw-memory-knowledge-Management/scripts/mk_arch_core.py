...
# ── LLM 后端（可选，懒加载） ────────────────────────────────────────────────
try:
    from llm_backend import LLMBackend, NoopLLMBackend, get_default_backend as _get_default_llm_backend
    _HAS_LLM_BACKEND_MODULE = True
except ImportError:
    _HAS_LLM_BACKEND_MODULE = False
    class LLMBackend:  # type: ignore[no-redef]
        def complete(self, prompt: str, max_tokens: int = 200, temperature: float = 0.0) -> Optional[str]:
            return None
        def is_available(self) -> bool:
            return False
    class NoopLLMBackend(LLMBackend):  # type: ignore[no-redef]
        pass
    def _get_default_llm_backend() -> "LLMBackend":
        return NoopLLMBackend()

_module_llm: Optional[LLMBackend] = None

# ── LLM 全局调用限频 ────────────────────────────────────────────────────────
_LLM_CALL_COUNT = 0
_LLM_CALL_LIMIT = 200  # 单次 session 最大 LLM 调用次数


def set_llm_backend(backend: "LLMBackend") -> None:
    """注入 LLM backend，影响所有 classify_bucket / chunk / quality_gate 等调用。"""
    global _module_llm
    _module_llm = backend


def set_llm_call_limit(limit: int) -> None:
    """设置单次 session 最大 LLM 调用次数。"""
    global _LLM_CALL_LIMIT
    _LLM_CALL_LIMIT = max(0, limit)


def get_llm_call_count() -> int:
    """返回当前 session 已使用的 LLM 调用次数。"""
    return _LLM_CALL_COUNT


def _get_llm() -> "LLMBackend":
    """返回当前 LLM backend。首次调用时自动发现并初始化。超限后返回 Noop。"""
    global _module_llm
    if _LLM_CALL_COUNT >= _LLM_CALL_LIMIT:
        return NoopLLMBackend()
    if _module_llm is None:
        if _HAS_LLM_BACKEND_MODULE:
            try:
                _module_llm = _get_default_llm_backend()
            except Exception:
                _module_llm = NoopLLMBackend()
        else:
            _module_llm = NoopLLMBackend()
    return _module_llm


def _llm_complete(prompt: str, max_tokens: int = 200, temperature: float = 0.0) -> Optional[str]:
    """带限频的 LLM 调用入口。超限后静默返回 None。"""
...
def get_llm_status() -> Dict[str, Any]:
    """返回 LLM 使用状态摘要，用于写入报告。"""
    return {
        "backend": get_llm_backend_name(),
        "call_count": _LLM_CALL_COUNT,
        "call_limit": _LLM_CALL_LIMIT,
        "limit_reached": _LLM_CALL_COUNT >= _LLM_CALL_LIMIT,
    }
...
def _llm_semantic_chunk(text: str) -> Optional[List[str]]:
    """调用 LLM 对无结构文本做语义边界切分，返回 chunk 列表；失败返回 None。"""
    if len(text.strip()) < 100:
        return None
    prompt = (
        "将以下文本切分为语义独立的记忆单元，每个单元是一条完整的、可独立检索的记忆。\n"
        "返回 JSON 数组，每个元素是切片文本字符串。文本不超过 800 字则直接返回单元素数组即可。\n\n"
        f"文本：{text[:2000]}"
    )
    result = _llm_complete(prompt, max_tokens=800, temperature=0.0)
    if not result:
        return None
    ...
    if start < 0 or end <= start:
        return None
    try:
        chunks = json.loads(cleaned[start:end + 1])
...
def split_markdown_sections(text: str, max_chars: int = 1200) -> List[Tuple[str, str]]:
    ...
    当文本缺乏 ## / ### 标题结构（标题少于 2 个）时，尝试 LLM 语义切块。
    """
    ...
    # LLM 语义切块：当标题数量少于 2 时（结构不明显），尝试 LLM 语义切分
    heading_count = sum(1 for l in text.splitlines() if l.startswith("## ") or l.startswith("### "))
    if heading_count < 2 and _get_llm().is_available() and len(text.strip()) > 100:
        llm_chunks = _llm_semantic_chunk(text)
        if llm_chunks and len(llm_chunks) >= 1:
            sections = [(f"chunk-{i+1}", c) for i, c in enumerate(llm_chunks)]
...
def extract_from_sqlite_file(path: Path, source_kind: str, max_records_per_file: int) -> List[SourceRecord]:
    ...
    try:
        ...
        for table in tables:
            # 安全校验：只允许合法 SQLite 标识符，防止注入
            if not re.fullmatch(r"[A-Za-z_\u4e00-\u9fff][A-Za-z0-9_\u4e00-\u9fff]*", table):
                continue
            quoted_table = '"' + table.replace('"', '""') + '"'
            cols_info = conn.execute(f"PRAGMA table_info({quoted_table})").fetchall()
            text_cols = []
            ...
            sql = f"SELECT rowid AS __rowid__, * FROM {quoted_table} LIMIT {per_table_limit}"
...
def classify_bucket(
    ...
    if source_kind == "knowledge":
        ...
        # P4: LLM 兜底 —— 当最高置信度 < 0.75 时，调 LLM 二次分类
        if conf < 0.75:
            llm_result = _llm_complete(
                f"记忆片段分类。从以下类别选一个最合适的：\n"
                f"profile（个人信息/身份）/ preferences（偏好/习惯）/ events（事件/经历）/ "
                f"agent_skill（技能/工具/操作方法）/ working（临时工作记录）/ "
                f"facts（知识事实）/ procedures（操作流程）/ references（资料链接）\n\n"
                f"记忆内容：{text[:500]}\n只返回类别名，不要解释。",
                max_tokens=30, temperature=0.0,
            )
            if llm_result:
                cat = llm_result.strip().lower().split()[0] if llm_result.strip() else ""
                cat = cat.strip(".,;:!?()[]{}'\"")
                mapped = map_category_to_bucket(source_kind, cat)
                if mapped:
                    return mapped, 0.82
    ...
    # P4: LLM 兜底 —— 当最高置信度 < 0.7 时，调 LLM 二次分类
    if conf < 0.7:
        llm_result = _llm_complete(
            f"记忆片段分类。从以下类别选一个最合适的：\n"
            f"profile（个人信息/身份）/ preferences（偏好/习惯）/ events（事件/经历）/ "
...
def _llm_extract_entities_and_relations(
    text: str,
    max_entities: int = 12,
    max_relations: int = 8,
    bucket: str = "",
) -> Optional[Tuple[List[str], List[Tuple[str, str, str]]]]:
    """用 LLM 联合抽取实体和关系。失败返回 None，由调用方 fallback 到规则。"""
    if len(text.strip()) < 40:
        return None
    prompt = (
        "从以下文本中抽取命名实体和关系。\n"
        "命名实体：人名、工具名、系统名、项目名、团队名、技术概念。排除通用词（如'系统'、'用户'、'配置'）。\n"
        "关系：(主体, 谓词, 客体) 三元组，谓词用简短动词/介词（如'负责'、'使用'、'属于'、'喜欢'）。\n\n"
        f"文本：{text[:1200]}\n\n"
        '返回 JSON：{"entities": ["实体1","实体2"], "relations": [["主体","谓词","客体"]]}。'
        "没有则返回空数组。只返回 JSON，不解释。"
    )
    result = _llm_complete(prompt, max_tokens=400, temperature=0.0)
    if not result:
        return None
    cleaned = result.strip()
    start = cleaned.find("{")
        ...
        for r in raw_relations:
            if isinstance(r, (list, tuple)) and len(r) >= 3:
                ...
                if s and p and o and s.lower() != o.lower():
                    relations.append((s, normalize_predicate(p), o))
...
def normalize_record(raw: SourceRecord, formation_mode: str = "manual") -> NormalizedRecord:
    ...
    # LLM 联合抽取实体+关系（失败时 fallback 到规则）
    llm_er = _llm_extract_entities_and_relations(structured_text, bucket=bucket)
    if llm_er is not None:
        entities, relations = llm_er
        # 用规则抽取补充 LLM 可能遗漏的实体
        rule_entities = extract_entities(structured_text)
        for re_ent in rule_entities:
            if re_ent not in entities and len(entities) < 16:
...
def should_reject_record(text: str) -> bool:
    """判断一条记录是否应被拒绝（不值得长期存储）。

    当现有规则不拒绝、且文本长度在 50-200 字时，调用 LLM 做最终质量判断。
    """
    text_stripped = text.strip() if text else ""
    text_len = len(text_stripped)
    if text_len < 30 or text_len > 500:
        return False
    result = _llm_complete(
        "以下文本是否值得长期存储为记忆？\n"
        "判断标准：\n"
        "- 值得保留：包含用户偏好、重要事件、技能知识、人物关系等有意义信息\n"
        "- 不值得保留：系统日志、格式化输出、临时调试信息、无实质内容\n\n"
        f"文本：{text_stripped[:300]}\n只返回 yes 或 no。",
        max_tokens=30, temperature=0.0,
    )
    if not result:
        return False
    answer = result.strip().lower().split()[0] if result.strip() else ""
    answer = answer.strip(".,;:!?()[]{}'\"")
    return answer == "no"
